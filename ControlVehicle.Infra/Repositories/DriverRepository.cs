...
public sealed class DriverRepository(VehicleDbContext db) : IDriverRepository
{
	...
	public async Task<Driver?> GetByCnh(Cnh cnh, CancellationToken ct = default)
		=> await _db.Drivers.SingleOrDefaultAsync(s => s.Cnh.Number == cnh.Number, ct);

	public async Task<Driver?> GetById(Guid id, CancellationToken ct = default)
		=> await _db.Drivers.SingleOrDefaultAsync(s => s.Id == id, ct);
